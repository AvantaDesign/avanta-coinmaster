...
import { 
  fetchRecurringFreelancers, 
  createRecurringFreelancer, 
  updateRecurringFreelancer, 
  deleteRecurringFreelancer 
} from '../utils/api';
import { formatCurrency, formatDate } from '../utils/calculations';

export default function RecurringFreelancersDashboard() {
  ...
  const loadFreelancers = async () => {
    try {
      ...
      const result = await fetchRecurringFreelancers(params);
  ...
  const handleEdit = (freelancer) => {
    setFormData({
      ...
      freelancer_rfc: freelancer.freelancer_rfc || '',
      amount: freelancer.amount.toString(),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await updateRecurringFreelancer(editingId, data);
      } else {
        await createRecurringFreelancer(data);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteRecurringFreelancer(id);
      loadFreelancers();
    } catch (err) {
  ...
  const handleStatusToggle = async (freelancer) => {
    try {
      ...
      await updateRecurringFreelancer(freelancer.id, { status: newStatus });
      loadFreelancers();
    } catch (err) {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            Pagos Recurrentes - Freelancers
          </h1>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Gestiona los pagos recurrentes a freelancers y colaboradores
          </p>
        </div>
        <button
        ...
        >
          {showForm ? 'Cancelar' : '+ Nuevo Freelancer'}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">
            {editingId ? 'Editar Freelancer' : 'Nuevo Freelancer'}
          </h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={formData.freelancer_rfc}
                onChange={(e) => setFormData({ ...formData, freelancer_rfc: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-slate-700 dark:text-white"
              />
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-gray-700">
                {freelancers.map((freelancer) => (
                  <tr key={freelancer.id} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
                        {freelancer.freelancer_name}
                      </div>
                      {freelancer.freelancer_rfc && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
                        {formatCurrency(freelancer.amount)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 dark:text-white">
                        {getFrequencyLabel(freelancer.frequency)}
                      </div>
                      {freelancer.payment_day && (
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          Día {freelancer.payment_day}
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 dark:text-white">
                        {freelancer.next_payment_date ? formatDate(freelancer.next_payment_date) : 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex gap-2">
                        ...
                        >
                          {freelancer.status === 'active' ? 'Desactivar' : 'Activar'}
                        </button>
                        <button
